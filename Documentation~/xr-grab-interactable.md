...
## Preventing or augmenting a grab interaction

When configuring a Grab Interactable for interaction with a variety of Interactors, you may want to only allow certain types of interactions, such as far or ray-based grabbing. This can be done in several ways. The simplest way is to update the [Interaction Layer Mask](xref:xri-interaction-layers) to match or include the Interaction Layer Mask of the Interactors you wish to allow interaction with.

Another approach that allows for further conditions and augmentation of the grab interaction is to write a custom [Interaction Filter](xref:xri-interaction-filters) or use a [Target Filter](xref:xri-target-filters) on the Interactor. The Interaction Filter can be scripted to only allow hovering and selecting with specific types of XR Interactors or under certain conditions, such as distance from the Grab Interactable, during character dialog, only when another Grab Interactable is placed into a specific Socket, or any number of conditions you require for your application. The [XR Target Filter](xref:xri-target-filters#xr-target-filter) component can be configured to filter the valid targets that an Interactor can interact with.
