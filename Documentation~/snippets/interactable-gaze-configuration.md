...
When you enable **Allow Gaze Assistance**, an [XR Gaze Interactor](xref:xri-xr-gaze-interactor) will place an [XR Interactable Snap Volume](xref:xri-xr-interactable-snap-volume) at this interactable to allow a properly configured [XR Ray Interactor](xref:xri-xr-ray-interactor) to snap to this interactable. See the [XR Interactable Snap Volume](xref:xri-xr-interactable-snap-volume) or [XR Ray Interactor](xref:xri-xr-ray-interactor) pages for further information about correctly configuring an `XRRayInteractor` to support an `XRInteractableSnapVolume`.
