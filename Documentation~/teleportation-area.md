...
| **Colliders** | The `Collider` components that define the teleportation surface this `TeleportationArea` will use. If this list is left empty, all Colliders on this GameObject and its children are used.<br />For these Colliders to be detected by teleport Interactors, both of the following conditions must be met:<ul><li>Their GameObjects are on Unity **physics layers** that are included in the Interactor’s **Raycast Mask**. For more information, see [XR Ray Interactor Raycast Configuration](xref:xri-xr-ray-interactor#raycast-config).</li><li>This Interactable’s **Interaction Layer Mask** shares at least one layer with the Interactor’s **Interaction Layer Mask**. For more information, see [Interaction layers](xref:xri-interaction-layers).</li></ul> |
