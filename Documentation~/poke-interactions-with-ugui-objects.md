...
Unlike [poking 3D objects](xref:xri-poke-3d), you don't need to add an [XR Simple Interactable](xref:xri-simple-interactable) or [XR Poke Filter](xref:xri-xr-poke-filter) to uGUI control game objects. Instead, add a [Tracked Device Graphic Raycaster](xref:xri-tracked-device-graphic-raycaster) to the `Canvas` and complete the [UI setup for XR interactions](xref:xri-ui-setup). With that setup, an [XRPokeInteractor](xref:xri-xr-poke-interactor) can hover and select controls in the canvas.
...
With the canvas and ray caster in place, you can add standard uGUI controls that respond to poke interactor input.
...
> [!NOTE]
> Disable the **Raycast Target** of any child Image or Text (TMP) components, such as the label text inside a button. When a **Raycast Target** is enabled on a child, it can intercept the ray cast meant for the parent button and prevent poke detection from working correctly.
